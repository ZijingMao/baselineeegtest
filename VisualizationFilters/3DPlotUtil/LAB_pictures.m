function current_frame = LAB_pictures(logic_values, channel_locs, freqs, width, height, sample)
...
for j=1:number_of_channels
   ...
   subplot('Position',[0.1 0.10 0.8 0.040]);
   barh(feature_samples,'white');
   hold on
   barh(epoh_time(sample),'black');
   axis([epoh_time(1) floor(max(epoh_time)) 0.6 1 ]);
   title(['PROGRESS ALONG SAMPLES:  ' int2str(sample) '/' int2str(feature_samples) ],'fontsize',18);
   set(gca,'fontsize',18);
   hold off   
