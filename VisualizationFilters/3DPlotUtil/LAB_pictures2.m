function current_frame = LAB_pictures2(logic_values, channel_locs, freqs, width, height, sample)
...
% plotmesh_modified EEGLAB function (by Arnaud Delorme, SCCN/INC/UCSD, 2003) 
...
for j=1:number_of_channels
    ...
    % Active Features
    if ( sum(logic_values(j,delta(1,1):delta(1,2),sample) == 1) > 0)
         cc1(j,1) = 1;
         cc1(j,2) = 0;
         cc1(j,3) = 0;
    ...
    % Active Features
    if ( sum(logic_values(j,theta(1,1):theta(1,2),sample) == 1) > 0)
        cc2(j,1) = 1;
        cc2(j,2) = 0;
        cc2(j,3) = 0;
        ...
        % Active Features
        if ( sum(logic_values(j,alpha(1,1):alpha(1,2),sample) == 1) > 0)
            cc3(j,1) = 1;
            cc3(j,2) = 0;
            cc3(j,3) = 0;
        ...
        % Active Features
        if ( sum(logic_values(j,beta(1,1):beta(1,2),sample) == 1) > 0)
            cc4(j,1) = 1;
            cc4(j,2) = 0;
            cc4(j,3) = 0;
   ...
   %------------------------- plot the figures --------------------
   movie_frame = figure('Position',[1 1 width height]);
   winsize = get(movie_frame,'Position'); % Create a Fullscreen empty window
   winsize(1:2) = [0 0];                  % Adjust size to include title, label, etc.
   set(movie_frame,'Color',[1 1 1]); 
   set(movie_frame, 'visible', 'off');
   ...
   %view([-204 12]);
   plotmesh_modified(TRI1, POS, NORM);
   hold on
   %view([-204 12]);
   ...
   hold on
   if (epoh_time(sample)>0)
       barh(max(epoh_time),'white');
       hold on
       barh(epoh_time(sample),'black');
   else
       barh(max(epoh_time),'white');
       hold on
       barh(epoh_time(sample),'white');
   end
   hold on
   ...
   % At the end, save the current frame and export to build the movie
   % outside the function
   current_frame = getframe(movie_frame, winsize);
   close 
   disp(['Frame ' int2str(sample) ' complete']);
