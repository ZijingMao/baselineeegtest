function [ labels ] = get_label_on_hit_miss(EEG, configs, events)
...
if responseLocked
    ...
    if ~isempty(ltTimeMinus20) && (ltTimeMinus20(end) < lastUrevent)
    ...
    if ~isempty(non_zero_event)
        ...
        if ((strcmp(next_urevent_type, allow_type) == 1) && (strcmp(label_event_type, target_type) == 1)) || ...
           ((strcmp(next_urevent_type, allow_type) == 0) && (strcmp(label_event_type, target_type) == 0))
                labels(1) = 1;
        else
            labels(1) = 2;
        end
...
for idx = 1:length(epoch)-1
    ...
    if ~responseLocked
        ...
        if ~isempty(ltTimePlus2) && (ltTimePlus2(1) > 1)
    ...
    if responseLocked            
        ...
        if ~isempty(non_zero_event)
            ...
            if (isTarget && (response_time < 2.0)) || (~isTarget && (response_time > 2.0))
                labels(idx) = 1;
            elseif isTarget && (response_time > 2.0) || (~isTarget && (response_time < 2.0))
                labels(idx) = 2;
            end
        elseif ~isTarget
            % No button press
